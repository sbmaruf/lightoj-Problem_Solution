...
int rec( int node , int from , int istake ){

    vis[node] =  1 ;
    if( G[node].size() == 0 ){
        return 0 ;
    }
    if( dp[node][istake] != -1 )return dp[node][istake] ;
    int res = 0 ;
    for( int i = 0 ; i < G[node].size() ; i++ ){
        int v = G[node][i] ;
        if( from == v )continue;
        if( istake==0 )res += max( rec(v,node,0) , rec(v,node,1)+1) ;
        else res += rec(v,node,0) ;
    }
    return dp[node][istake] = res ;
